...
def main() -> None:
    ...
    for iteration in range(1, args.max_iter + 1):
        ...
        status = str(feedback_json.get("status", "")).lower()
        rubric = feedback_json.get("rubric", {}) if isinstance(feedback_json, dict) else {}
        criteria_met = str(rubric.get("criteria_met", "")).lower() == "true"
        if status == "success" or criteria_met:
            break
